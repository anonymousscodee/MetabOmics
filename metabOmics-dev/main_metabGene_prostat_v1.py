...
if __name__ == "__main__":
    ...
    # Read data
    X_breast, y_breast = SkUtilsIO("../Dataset/metabData_prostat.csv").from_csv(label_column='Factors')
    X_tr, y_tr = SkUtilsIO("../Dataset/geneData_prostat.csv").from_csv(label_column='Factors')
    ...
    dosya_adi = "prostat_pathways_v1.txt"
    count = 1
    ...
    with open(dosya_adi, 'w') as json_file:
        ...
        for train_index, test_index in skf.split(X_breast, y_breast):
            try :
                ...
                break
    with open('scores_prostat_v1.txt', 'w') as f:
        try:
