...
with open('Databases/recon3D_uniprot_mapping.json', 'r') as json_file:
    uniprot_synonym_mapping = json.load(json_file)
...
if __name__ == '__main__':
        ...
        if r.gene_reaction_rule != '':
            universal_graph.add_vertex(
                ...
                vert_info={
                    ...
                    'gpr': parse_gpr_expression(str(r.gpr), uniprot_synonym_mapping)
                },
                omic_type='R'
            )
            for gen in r.genes:
                unique_gene.add(gen.id)
                if gen.id in uniprot_synonym_mapping:
                    uniprot_id = uniprot_synonym_mapping[gen.id]
                    label = [uniprot_id]
                else:
                    nogene.add(gen.id)
                    label = []
    ...
    universal_graph.save_to_json("Databases/universalGraph_unFiltered.json")
    ...
    universal_graph.save_to_json("Databases/universalGraph_Filtered.json")
    end = time.time()
    ...
    json_file_path = 'Databases/universalGraph_elapsedTime.txt'
